...
export const TransferCard: React.FC = () => {
  ...
  const handleTransfer = async () => {
    ...
    if (!recipient || !amount) return alert('Check details');

    try {
      setIsLoading(true);
      await transfer({id: recipient, amount, sendTransaction, queryClient, connection});
    } catch (err) {
      console.error(err);
  ...
  return (
    <Card className='w-80 p-6 bg-active-gradient flex flex-col gap-4'>
      <div className='flex flex-col gap-2'>
          <Input placeholder="input address to transfer funds" onChange={e => setRecipient(e.target.value)}/>
